...
# ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
for ixz1 in range(1,NDz1+1):
    for jxz1 in re.do(0, NDz1):
        HCz1[jxz1, ixz1] = z0z1
#@ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @
exit(1)
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
for imz1 in re.do(0, NDz1-1):
...
WBz1 = NDz1
yyz1 = 0
for iyz1 in re.do(1, WBz1):
    yy1z1 = yyz1 + iyz1
    for imz1 in re.do(1, 1):
...
for iyz1 in re.do(1, WBz1):
    yy1z1 = yyz1 + iyz1
    for imz1 in re.do(1, 1):
        HQPC1z1[yy1z1 + (imz1 - 1) * NDz1, yy1z1 + (imz1 - 1) * NDz1] = EAz1 * zrz1
...
for iyz1 in re.do(1, WBz1+1):
    yy1z1 = yyz1 + iyz1
    for imz1 in re.do(1, 2):
...
for iaz1 in re.do(0, NDz1):
    for jz1 in re.do(0, NDz1):
        MDUXz1[jz1, iaz1] = z0z1

for ibz1 in re.do(0, NDz1):
...
# ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
piz1 = 0.
for iyz1 in re.do(0, 1):

    for ixz1 in re.do(0, 1):
        vv1z1 = 1
        ...
        #  ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** *
        #  ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** *
        for inz1 in re.do(1, NDz1+1):
            vvz1 = 0
            for jnz1 in re.do(1, NDz1+1):
                vvz1 = 1 + vvz1
                CC1z1 = np.sqrt((2.0 / (NDz1+1)))
                piz1 = 3.141592654
                UUz1[jnz1-1, inz1-1] = CC1z1 * np.sin(piz1 * vvz1 * inz1 / (NDz1 + 1))

        for ivz1 in re.do(1, NDz1+1):
...
#CALCULAR A MATRIZ MDUXz SIMILAR QUE A CALCULADA GUz(funcao de Green contato)
        ...
        XIz1 = re.invxz1(XIz1, NDz1)
        GIz1 = 1E+30 * XIz1
        QW1 = GIz1

        jxz1 = 0

        for imz1 in re.do(MCz1, 0, -1):
            # ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** *
            #  ** ** ** ** ** ** ** ** DISPOSITIVO ** ** ** ** ** ** ** ** **
            jyz1 = 0
            jxz1 = jxz1 + 1
            for nxz1 in re.do(1, NDz1+1):
                jyz1 = jyz1 + 1
        ...
        if ixz1==0:
            fileds = open("dis.dat", "w")
            dis = DISPOz1
            fileds.write(str(DISPOz1))
            fileds.flush()
            os.fsync(fileds)
            fileds.close()
        for mxz1 in re.do(1, NMz1+1):
            for nxz1 in re.do(1, NDz1+1):
        ...
        RRuvz1 = 0.
        TTz1 = 0.
        for IKz1 in re.do(1, NDz1+1):
            vz1 = IKz1

            for IK1z1 in re.do(1, NDz1+1):
                uz1 = IK1z1
                Evz1 = 2. * VAz1 - 2. * VAz1 * np.cos(piz1 * vz1 / (NDz1 + 1))
                Euz1 = 2. * VAz1 - 2. * VAz1 * np.cos(piz1 * uz1 / (NDz1 + 1))
                if 0 <= Ez1 < Evz1:
                    CF1z1 = np.sin(np.arccos((Ez1 - Evz1) / (2. * VAz1) + 1.))
                else:
                    CF1z1 = np.sin(np.arccos((Ez1 - Evz1) / (2. * VAz1) - 1.))

                if (Ez1 >= 0) and (Ez1 < Euz1):
                    CF2z1=np.sin(np.arccos((Ez1-Euz1) / (2. * VAz1)+1.))
                else:
                    CF2z1=np.sin(np.arccos((Ez1-Euz1) / (2. * VAz1)-1.))

                tuvz1=2. * VAz1 * np.sqrt(abs(CF2z1 * CF1z1)) * ziz1 * (GTvz1[uz1-1, vz1-1])
